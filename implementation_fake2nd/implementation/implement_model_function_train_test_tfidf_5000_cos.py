...
import numpy as np
from feature_helpers.feature_generator import make_tfidf_combined_feature_cos_5000, load_tfidf_y
...
def MLP_Classifier(row_body_train, row_stance_train, row_body_test, row_stance_test,
    ...
    n_classes = 4
    if mode == 'train':
        X_train = make_tfidf_combined_feature_cos_5000(row_body_train, row_stance_train, head_dir_train, body_dir_train,
                                                       label_dir_train, cos_dir_train)
        y_train = load_tfidf_y(label_dir_train)
        # X_train = shuffle_dataset(seed, X_train)
        # y_train = shuffle_dataset(seed, y_train)
        if holdout:
            X_test = make_tfidf_combined_feature_cos_5000(row_body_test, row_stance_test, head_dir_test, body_dir_test,
                                                          label_dir_test, cos_dir_test)
...
if __name__ == "__main__":
    ...
    cos_dir_train = '../pickled_model/tfidf_cos_train.pkl'
    ...
    cos_dir_test = '../pickled_model/tfidf_cos_test.pkl'
    ...
    MLP_Classifier(row_body_train, row_stance_train, row_body_test, row_stance_test,
                   ...
                   learning_rate=0.001, batch_size=188,
                   training_epoch=70,
                   init_bias=0.001, mode='test', save_model_path='../tf_model/tfidfcos_5000_epoch', holdout=False)
