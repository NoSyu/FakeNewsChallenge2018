...
import numpy as np
from feature_helpers.feature_generator import make_NMF_300_feature, load_tfidf_y
...
def MLP_Classifier(row_body_train, row_stance_train, row_body_test, row_stance_test,
                   head_tfidf_dir_train, body_tfidf_dir_train, label_dir_train,
                   head_tfidf_dir_test, body_tfidf_dir_test, label_dir_test,
                   nmf_model_path, nmf_head_train_path, nmf_body_train_path,
                   nmf_head_test_path, nmf_body_test_path,
                   cos_dist = False,
    ...
    :param head_tfidf_dir_train: head train pkl 파일이 있는 경로
    :param body_tfidf_dir_train: body train pkl 파일이 있는 경로
    :param label_dir_train: y train label pkl 파일이 있는 경로
    :param cos_dir_train: cos_tfidf_train pkl 파일이 있는 경로
    :param head_tfidf_dir_test: head test pkl 파일이 있는 경로
    :param body_tfidf_dir_test: body test pkl 파일이 있는 경로
    ...
    n_classes = 4
    if mode == 'train':
        X_train = make_NMF_300_feature(row_body_train, row_stance_train, head_tfidf_dir_train, body_tfidf_dir_train,
                                       label_dir_train, nmf_model_path, nmf_head_train_path, nmf_body_train_path, cos_dist)
        y_train = load_tfidf_y(label_dir_train)
        # X_train = shuffle_dataset(seed, X_train)
        # y_train = shuffle_dataset(seed, y_train)
        if holdout:
            X_test = make_NMF_300_feature(row_body_test, row_stance_test, head_tfidf_dir_test, body_tfidf_dir_test,
                                          label_dir_train, nmf_model_path, nmf_head_test_path, nmf_body_test_path, cos_dist)
...
if __name__ == "__main__":
    ...
    nmf_model_path = '../pickled_model/nmf_all_model.pkl'
    nmf_head_train_path = '../pickled_model/nmf_head_train_model.pkl'
    nmf_body_train_path = '../pickled_model/nmf_body_train_model.pkl'

    nmf_head_test_path = '../pickled_model/nmf_head_test_model.pkl'
    nmf_body_test_path = '../pickled_model/nmf_body_test_model.pkl'

    # save_model_path = '../tf_model/nmf_300_epoch'
    save_model_path = '../tf_model/nmf_300_cos_epoch'
    MLP_Classifier(row_body_train, row_stance_train, row_body_test, row_stance_test,
                   ...
                   nmf_model_path, nmf_head_train_path, nmf_body_train_path,
                   nmf_head_test_path, nmf_body_test_path, cos_dist=True,
                   learning_rate=0.001, batch_size=188,
                   training_epoch=70,
