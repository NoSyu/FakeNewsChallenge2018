...
import numpy as np
from feature_helpers.feature_generator import make_tfidf_combined_feature, load_tfidf_y
...
def MLP_Classifier(row_body_train, row_stance_train, row_body_test, row_stance_test,
    ...
    n_classes = 4
    if mode == 'train':
        X_train = make_tfidf_combined_feature(row_body_train, row_stance_train, head_dir_train, body_dir_train,
                                              label_dir_train)
        y_train = load_tfidf_y(label_dir_train)
        # X_train = shuffle_dataset(seed, X_train)
        # y_train = shuffle_dataset(seed, y_train)
        if holdout:
            X_test = make_tfidf_combined_feature(row_body_test, row_stance_test, head_dir_test, body_dir_test,
                                                 label_dir_test)
        ...
        X_test = make_tfidf_combined_feature(row_body_test, row_stance_test, head_dir_test, body_dir_test, label_dir_test)
        y_test = load_tfidf_y(label_dir_test)
