...
from keras_model.utils.testDataset import TestDataSet
from keras_model.utils.generate_test_splits import kfold_split

def get_head_body_tuples_test():

    data_path = "%s/data/" % path.dirname(path.dirname(path.dirname(path.abspath(__file__))))
...
def word_ngrams_concat_count_no_bleeding(headlines, bodies, headlines_test, bodies_test,
                                         binary=False, max_features=100, ngram_range=(1, 1),
...
def word_ngrams_concat(headlines, bodies, max_features=100, ngram_range=(1, 1),
...
if __name__ == '__main__':
    ## make MLP features
    headlines, bodies = get_head_body_tuples(include_holdout=False)
    headlines_test, bodies_test = get_head_body_tuples_test()
    X_train, X_test = word_ngrams_concat_tfidf_no_bleeding(headlines, bodies, headlines_test, bodies_test, use_idf=True)
    print('X_train_shape : ', X_train.shape)
    print('X_test_shape : ', X_test.shape)
    # print(X_train.tolist(), X_test.tolist())

    pickle.dump(X_train, open('../../features/tfidf_head_body_200_train.pkl', 'wb'), pickle.HIGHEST_PROTOCOL)
    pickle.dump(X_test, open('../../features/tfidf_head_body_200_test.pkl', 'wb'), pickle.HIGHEST_PROTOCOL)
